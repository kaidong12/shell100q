        ...
        stage('Execute') {
            steps {
                ...
                sh """
                    ...
                    virsh list --all
                    ping -c 5 10.0.99.15
                    grep -A 30 pm5: /home/tester/yaml/${params.YAML_PATH}/Pairwise_keying.yaml
                    date
                    echo "============================environment============================"
                    
                    echo "----------------------------------------------------------------------------------------------------"
                    # /home/tester/vtest/bin/runner pwk -v -ntr -ntg -ns -pdvc -m Pairwise_keying -t setup -t P0 -q;
                    /home/tester/vtest/bin/runner pwk -v -ntr -y /home/tester/yaml/${params.YAML_PATH}/Pairwise_keying.yaml -ntg -ns -pdvc -m Pairwise_keying -n tb -o -b next -t setup -t P0 -q;
                    echo "----------------------------------------------------------------------------------------------------"
                    date
