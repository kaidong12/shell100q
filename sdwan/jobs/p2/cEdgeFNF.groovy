        ...
        stage('Execute') {
            steps {
                ...
                sh """
                    ...
                    virsh list --all
                    ping -c 5 10.0.99.15
                    grep -A 30 pm5: /home/tester/yaml/${params.YAML_PATH}/CEDGE_FNF_sdwan_init.yaml
                    date
                    echo "============================environment============================"
                    
                    echo "----------------------------------------------------------------------------------------------------"
                    # /home/tester/vtest/bin/runner cEdgeFNF -ntr -ntg -ns -ntg -ivm -ns -m CEDGE_FNF -t setup -t P0 -q;
                    /home/tester/vtest/bin/runner cEdgeFNF -ntr -ntg -ns -ntg -ivm -ns -m CEDGE_FNF -y /home/tester/yaml/${params.YAML_PATH}/CEDGE_FNF_sdwan_init.yaml -n tb -o -b next -t setup -t P0 -q;
                    echo "----------------------------------------------------------------------------------------------------"
                    date
