        ...
        stage('Execute') {
            steps {
                ...
                sh """
                    ...
                    virsh list --all
                    ping -c 5 10.0.99.15
                    grep -A 30 pm5: /home/tester/yaml/${params.YAML_PATH}/sdwan_init_platform_usb_disable_pm5.yaml
                    date
                    echo "============================environment============================"
                    
                    echo "----------------------------------------------------------------------------------------------------"
                    # /home/tester/vtest/bin/runner.py config_group_platform_usb_disable -ns -ntr -ntg -ivm -v -m usb_disable;
                    /home/tester/vtest/bin/runner config_group_platform_usb_disable -ns -ntr -ntg -m usb_disable -y /home/tester/yaml/${params.YAML_PATH}/sdwan_init_platform_usb_disable_pm5.yaml -n tb -b next -o;
                    echo "----------------------------------------------------------------------------------------------------"
                    date
